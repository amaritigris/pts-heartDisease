...
create_pts <- function(dataset, nreps = 100, sampsize = 50, endclass = 4) {
  ...
  # Apply constraints to penalize backward transitions in disease stage
  ...
  for (i in 1:nreps) {
    ...
    startp <- which(class[mysamp] == min(class[mysamp]))[1]
    endp <- which(class[mysamp] == max(class[mysamp]))[1]
...
ggplot(combined_pts, aes(x = a, fill = factor(c))) +
  ...
  labs(
    title = "Density Plot of Age Distribution by Disease Stage (Faceted by Gender)",
...
ggplot() +
  ...
  labs(title = "Relationship Between Disease Stage and Resting Blood Pressure (Male vs Female)",
       x = "Resting Blood Pressure", y = "Disease Stage") +
  theme_minimal()
