...
# Function to generate PTS with disease stage constraints
generate_pts_stage <- function(features, stage, nreps = 50, sampsize = 30, endclass = 4) {
  ...
  # Apply constraints to penalize backward transitions in disease stages
  for (i in 1:nrow(features)) {
    for (j in 1:nrow(features)) {
      if (stage[j] < stage[i]) {
        cons[i, j] <- 999  # Penalize backward disease stage transitions
      }
    }
  }
  ...
  for (i in 1:nreps) {
    repeat {
      # Sample indices ensuring the presence of both start (earliest stage) and end (endclass) stages
    ...
    # Identify start (earliest stage) and end (latest stage) points
    startp <- which(stage[mysamp] == min(stage[mysamp]))[1]
    endp <- which(stage[mysamp] == max(stage[mysamp]))[1]
...
# Generate PTS for males and females based on disease stage constraints
male_pts_stage <- generate_pts_stage(male_data[, 1:2], male_data$Class)
female_pts_stage <- generate_pts_stage(female_data[, 1:2], female_data$Class)
...
ggplot() +
  ...
  lapply(male_pts_stage, function(pt) {
    geom_path(data = male_data[pt, ], aes(x = X, y = Y), color = "black", alpha = 0.4, size = 0.1)
  }) +
  labs(
    title = "PTS Trajectories for Male Patients (Stage Constraints)",
    x = "PCA Component 1", y = "PCA Component 2", color = "Class"
  ) +
  theme_minimal()
...
ggplot() +
  ...
  lapply(female_pts_stage, function(pt) {
    geom_path(data = female_data[pt, ], aes(x = X, y = Y), color = "black", alpha = 0.4, size = 0.1)
  }) +
  labs(
    title = "PTS Trajectories for Female Patients (Stage Constraints)",
    x = "PCA Component 1", y = "PCA Component 2", color = "Class"
  ) +
  theme_minimal()
