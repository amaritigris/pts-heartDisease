...
for (i in 1:sampsize) {
  for (j in 1:sampsize) {
    if (age[j] < age[i]) {
      cons[i, j] <- 999  # High penalty for age-reversing transitions
    }
  }
}
...
pts_df <- do.call(rbind, lapply(1:20, function(i) {  # Select only 20 PTS for clarity
  data.frame(
...
ggplot() +
  ...
  # Labels & Theme
  labs(title = "Comparison of Real vs. Pseudo-Time Series (Age Constraints)",
